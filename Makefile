...
# $(1) = pio env name (e.g. HW1_0), used verbatim as the dist label.
# $(2) = version
define copy_uf2
	@mkdir -p $(DIST_DIR)
	@cp .pio/build/$(1)/firmware.uf2 $(DIST_DIR)/PocketPD_$(1)-v$(2).uf2
	@echo "→ $(DIST_DIR)/PocketPD_$(1)-v$(2).uf2"
endef
...
build-v2:
	pio run -e HW1_3
	$(call copy_uf2,HW1_3,$(FIRMWARE_VERSION))

build-hw1_0:
	pio run -e HW1_0
	$(call copy_uf2,HW1_0,$(FIRMWARE_VERSION))

build-hw1_1:
	pio run -e HW1_1
	$(call copy_uf2,HW1_1,$(FIRMWARE_VERSION))
